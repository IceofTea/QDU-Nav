...
>
> **🏷️ 当前版本：v1.1.1**（功能迭代版。版本号在 `src/config/site.js` 中维护，页脚与「关于本站」面板同步展示）
...
| 🗓️ 课程表 | **真实课表查询**（班级 / 教室 / 教师 → 周课表），支持**学期切换**与**按周次筛选**；班级列表**全量展示**（不再截断）；数据来自教务处课程总表，**合并最近 7 个学期**（约 5 万条排课，教室并集 780+ 间） |
| 🧭 教室导航 | 空教室查询（结果独立页）+ 教室一周占用 + 教学楼导航指引 |
| 📅 校历 | 官方校历入口 + 学期时间线 |
| 🍽️ 食堂空座率 | 官方食堂名单 + 营业时段实时判定 + 真实档口/招牌菜 |
| 🪪 新生学号查询 | 官方录取查询入口与流程引导（录取系统需校内环境，不做代查） |
| 📊 数据洞察 | 基于近 7 学期 5 万条排课的统计：热门教室 / 教师 / 课程、学期趋势、周节次分布，并展示**课程性质 / 校区 / 学院开课分布**（Python 聚合；分布数据随定时抓取补充） |
...
| 🎡 美食轮盘 · 🧩 教学楼速配 | 校园小游戏 |
| 📣 贴吧舆情 | 爬取**青岛大学吧**公开列表页的轻量舆情分析：热帖榜 / 关键词 / 话题分布 / 近 14 天发帖趋势（贴吧反爬较严时如实降级，附官方入口；数据由定时任务尽力抓取） |
...
| **v1.1.0** | **功能迭代版**。①数据链路修复：快照生成保留 `col/campus/kind/credit` 等附带列，「数据洞察」新增**课程性质 / 校区 / 学院开课分布**与解读文案（旧快照为「未标注」，下次定时抓取后自动补充真实分布）；②课程表班级列表**全量展示**（移除 40 条截断）；③生活费计数器增强：类别扩充、**微信/支付宝账单 CSV 导入**（自动归类）、**奖学金/助学金预设一键带出**、结余情绪反馈分档 + 支出月对比；④青大知多少题库扩至 **102 题**、答题选项徽章化、成绩页**错题回顾**与**本机排行榜**；⑤今天吃什么支持**校区筛选**；⑥删除「受福师大启发」等无关元素；⑦新增**贴吧舆情**：`tieba.py`/`tieba.mjs` 尽力抓取「青岛大学吧」列表页产出热帖榜 / 关键词 / 话题分布 / 近 14 天趋势（反爬失败时如实降级并保留旧数据），**替换原本地演示版评论区**；⑧学校官网：**学院官网按学科分类**（人文社科 / 理工 / 医学 / 艺术与体育 / 合作办学），社区与资源新增**青岛大学吧**入口。 |
