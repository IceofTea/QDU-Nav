...
| 🎡 美食轮盘 · 🧩 教学楼速配 | 校园小游戏 |
| 💬 评论区 | 本地评论墙（昵称 / 游客 + 敏感词过滤，纯静态无后端，仅存本机） |
...
| 版本 | 说明 |
| --- | --- |
| **v1.1.0** | **功能迭代版**。①数据链路修复：快照生成保留 `col/campus/kind/credit` 等附带列，「数据洞察」新增**课程性质 / 校区 / 学院开课分布**与解读文案（旧快照为「未标注」，下次定时抓取后自动补充真实分布）；②课程表班级列表**全量展示**（移除 40 条截断）；③生活费计数器增强：类别扩充、**微信/支付宝账单 CSV 导入**（自动归类）、**奖学金/助学金预设一键带出**、结余情绪反馈分档 + 支出月对比；④青大知多少题库扩至 **102 题**、答题选项徽章化、成绩页**错题回顾**与**本机排行榜**；⑤今天吃什么支持**校区筛选**；⑥删除「受福师大启发」等无关元素；⑦新增**评论区**（本地评论墙：昵称/游客 + 敏感词过滤）。 |
