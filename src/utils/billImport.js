...
const KEYWORDS = [
  { key: 'food', words: ['食堂', '餐', '饭', '外卖', '奶茶', '咖啡', '果', '零食', '面', '饺', '烧烤', '火锅', '汉堡', '小吃', '买菜', '菜场', '餐厅', '麦乐送', '烘焙', '蛋糕', '早餐', '粉', '麻辣', '香锅', '米线', '肠', '饼', '牛奶', '甜品', '糖', '饿了么', '美团', '肯德基', '麦当劳', '瑞幸', '蜜雪', '星巴克', '必胜客'] },
  { key: 'party', words: ['聚餐', '团建', '约饭', '啤酒', '宵夜', '串串', '烧烤店', '火锅店', 'KTV', 'ktv', '酒吧'] },
  { key: 'transport', words: ['公交', '地铁', '打车', '滴滴', '高铁', '火车', '共享', '加油', '停车', '飞机', '高速', '停车费', '地铁卡', '公交卡', '铁路', '航空', '加油卡', '高德', '哈啰', '青桔', '美团打车', 'T3出行', '曹操出行'] },
  { key: 'beauty', words: ['理发', '美发', '美容', '美甲', '护肤', '面膜', '造型', '染发', '剪发', 'spa', '海蓝之谜', '兰蔻', '雅诗兰黛'] },
  { key: 'digital', words: ['手机', '电脑', '数码', '耳机', '充电', '硬盘', '键盘', '家电', '电器', '显示器', '平板', '摄像头', '路由器', '智能', '华为支付', '苹果', '小米', '联想', '戴尔', '华硕'] },
  { key: 'sport', words: ['健身', '运动', '球', '羽毛球', '乒乓球', '跑步', '瑜伽', '游泳', '健身房', '户外', '登山', '篮球', '足球', '骑行', '迪卡侬', '耐克', '阿迪'] },
  { key: 'virtual', words: ['游戏', 'steam', '会员', '点卡', 'vip', '王者', '原神', '皮肤', '月卡', '视频会员', 'b站', 'bilibili', '腾讯视频', '爱奇艺', '优酷', '网易云', 'qq音乐', '酷狗'] },
  { key: 'housing', words: ['房租', '水费', '电费', '燃气', '物业', '宿舍', '宽带', '热水', '酒店', '如家', '民宿', '公寓', '宾馆', '汉庭', '贝壳', '链家'] },
  { key: 'study', words: ['书', '教材', '打印', '文具', '资料', '报名', '考研', '考证', '学费', '课程', '网课', '得到', '知乎', '慕课'] },
  { key: 'cloth', words: ['衣', '服', '鞋', '裤', '帽', '穿搭', '卫衣', '外套', '裙', '优衣库', 'zara', 'h&m', '耐克', '阿迪', '李宁', '安踏'] },
  { key: 'medical', words: ['药', '医院', '挂号', '体检', '口罩', '诊所', '门诊', '药店', '同仁堂', '海王星辰', '大参林'] },
  { key: 'daily', words: ['洗发', '纸巾', '洗衣', '牙膏', '毛巾', '日用', '生活', '洗涤', '纸', '超市', '便利店', '711', '全家', '罗森', '名创优品'] },
  { key: 'phone', words: ['话费', '流量', '移动', '联通', '电信', '校园网', '宽带', '缴费', '中国移动', '中国联通', '中国电信'] },
  { key: 'fun', words: ['电影', 'ktv', '演出', '门票', '密室', '桌游', '景区', '乐园', '猫眼', '大麦', '影院', 'ktv', '大众点评', '马蜂窝'] },
  { key: 'transfer', words: ['转账', '红包'] }
]

const INCOME_KEYWORDS = [
  { key: 'invest', words: ['利息', '结息', '理财', '余额宝', '零钱通', '收益', '分红'] },
  { key: 'transfer', words: ['转账', '汇款', '转入', '代发', '工资', '薪'] },
  { key: 'prize', words: ['红包', '奖金', '奖励', '签到', '金币', '积分', '提现', '佣金', '返现', '活动', '抖音', '快手', '平台', '中奖'] },
  { key: 'parttime', words: ['兼职', '打工', '实习', '劳务', '报酬'] },
  { key: 'scholarship', words: ['奖学金', '助学金', '补贴', '补助'] },
  { key: 'resell', words: ['闲鱼', '转转', '二手', '转让', '出售'] },
  { key: 'refund', words: ['退款', '退货', '售后'] },
]
...
async function parseCcbBill(bytes) {
  ...
  for (let i = headIdx + 1; i < rows.length; i++) {
    ...
    if (kind === 'income') {
      ...
      else cat = guessIncomeCat(`${note} ${summary}`)
  ...
  for (let i = headIdx + 1; i < rows.length; i++) {
    ...
    if (brand === 'alipay') {
      if (kind === '收入') {
        ...
        else cat = guessIncomeCat(`${name} ${party}`)
      } else {
...
async function parseBocBill(bytes) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/跨行转账|跨行转出/.test(summary) && kind === 'expense') {
      ...
      if (/用户/.test(nextLines)) { skipped.neutral++; continue }
    }
    let cat = 'other'
    if (kind === 'income') {
      if (/结息/.test(summary)) cat = 'invest'
      else if (/银联入账|跨行转账/.test(summary)) cat = 'transfer'
      else cat = guessIncomeCat(`${party} ${summary}`)
    } else {
