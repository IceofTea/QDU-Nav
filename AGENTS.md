...
| 日期 | 版本 | 内容 |
| --- | --- | --- |
| 2026-08-17 | v1.2.9 | **统计功能**。新增 `src/components/VisitStats.vue`：首页「关于本站」面板内、贡献者墙按钮下方展示**独立访客 UV / 累计访问 PV**，数据来自**自建独立计数服务** `counter/server.mjs`（无依赖 Node 单文件，cookie 去重 UV + 落盘持久化，CORS 开放，可部署到 Render/Glitch 等免费平台，部署地址填入 `src/config/site.js` 的 `SITE.counter.api`）；与 QDU-Wiki 的 Vercount 统计完全独立（实测 Vercount 无页面级 UV，同域下两站无法独立，故自建）。sessionStorage 缓存先行回填 + 8s 超时 + 服务不可用时降级显示「—」。验证：构建通过、回归 v12–v19 全绿、本地 counter 实测（首页渲染 独立访客/累计访问 数字 + 「自建独立计数」标注 + 卡片位于贡献者墙按钮下方）。 |
