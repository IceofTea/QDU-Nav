...
| 日期 | 版本 | 内容 |
| --- | --- | --- |
| 2026-08-17 | v1.2.9 | **统计功能**。新增 `src/components/VisitStats.vue`：首页「关于本站」面板内展示**独立访客 UV / 累计访问 PV**（Vercount 免费统计，与 QDU-Wiki 同源，站点级累计）；sessionStorage 缓存先行回填 + 5s 节流防连发 + 8s 超时 + 失败隐藏降级。验证：构建通过、回归 v12–v19 全绿、浏览器实测（about 面板渲染 独立访客/累计访问 数字 + 标注「本站累计 · 由 Vercount 统计」）。 |
