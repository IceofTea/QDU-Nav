...
| 日期 | 版本 | 内容 |
| --- | --- | --- |
| 2026-08-17 | v1.3.1 | **校领导测试**。参考 leadertest.site 多维原型比对机制改造为青大版：`src/data/leaders.js`（DIMS 9 维：决策魄力/规划理性/改革锐气/情怀感召/制度规范/育人理念/师生动员/执行强度/发展导向；leaders 8 原型：竺苗龙/徐建培/夏临华/王安民/范跃进/夏东伟/胡金焱/魏志强，各含 period/role/bio/summary/vec(0-10)/weight/photo；questions 11 题校园管理场景，binary/single/policy/likert 各题型带维度 score）+ `src/views/LeaderTest.vue`（intro→quiz→result 三阶段，加权距离匹配，结果页含唯一匹配/多维比对条/为什么是这个人/简要事迹/最接近两位/分享文案复制/准·一般·不准反馈/再测一次，照片缺失回退首字母渐变头像）；注册 apps.js（游戏组）与 router.js。验证：构建通过、回归 v12–v19 全绿（v19 版本断言放宽为 v1.x.y）、375px 手机视口实测完整答题到结果（匹配竺苗龙、比对条/分享/再测齐全）。 |
