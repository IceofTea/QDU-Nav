...
| 日期 | 版本 | 内容 |
| --- | --- | --- |
| 2026-08-17 | v1.2.9 | **统计功能**。新增 `src/components/VisitStats.vue`：首页「关于本站」面板内、贡献者墙按钮下方展示**独立访客 UV / 累计访问 PV**，数据来自**自建独立计数服务**——`counter/server.ts`（Deno Deploy 版，`Deno.serve` + `Deno.openKv` 持久化）与 `counter/server.mjs`（Node 版，落盘 `counter/data.json`，已在 .gitignore）；UV 去重由前端 localStorage + `isNewUv` 参数完成（跨域第三方 Cookie 可能被拦截）。部署：dash.deno.com → Sign in with GitHub → New Project → 连本仓库 → Main Entrypoint 填 `counter/server.ts` → Deploy 后复制 `https://xxx.deno.dev` 填入 `src/config/site.js` 的 `SITE.counter.api`。与 QDU-Wiki 的 Vercount 统计完全独立（实测 Vercount JS 无 `page_uv` 锚点回填、site_uv 按域名共享，同域两站无法独立，故自建）。验证：构建通过、回归 v12–v19 全绿（v13 统计卡片断言）、本地实测（首次 uv+1、二次仅 pv+1，localStorage 去重生效；卡片位于贡献者墙按钮下方）。 |
